...
function CheckoutForm({ planId }: { planId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto p-6 bg-[var(--color-card)] rounded-xl shadow-lg border border-[var(--color-border)] transition-colors duration-300">
      <h3 className="text-xl font-semibold text-[var(--color-foreground)] mb-4">Dados de Pagamento</h3>
      <div className="p-3 border border-[var(--color-border)] rounded-md bg-[var(--color-background)]">
        {/* <CardElement options={{ hidePostalCode: true }} /> */}
        <p className="text-sm text-[var(--color-muted-foreground)] text-center">Formulário Seguro do Stripe será renderizado aqui.</p>
      </div>
