...
export default function MeusPlanosPage() {
  ...
  if (loading) {
    return <LoadingState label="Carregando planos..." description="Preparando as melhores opções para você" />;
  }

  return (
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-[var(--color-background)] animate-in fade-in duration-700">
      <div className="mb-10 text-center">
        <h1 className="text-4xl font-extrabold text-[var(--color-foreground)] tracking-tight mb-2">Meus Planos</h1>
        <p className="text-lg text-[var(--color-muted-foreground)]">Escolha o plano perfeito para impulsionar suas vendas de energia solar.</p>
      </div>

      <div className="grid md:grid-cols-2 gap-8 mb-16">
        {MOCK_PLANS.map(plan => (
          <div key={plan.id} className="bg-[var(--color-card)] rounded-2xl shadow-xl overflow-hidden border border-[var(--color-border)] hover:shadow-2xl transition-all duration-300 flex flex-col hover:-translate-y-1">
            <div className="p-8 border-b border-[var(--color-border)] bg-[var(--color-muted)]/30">
              <h3 className="text-2xl font-bold text-[var(--color-foreground)] mb-2">{plan.name}</h3>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-extrabold text-[var(--color-primary)]">R$ {plan.price.toFixed(2)}</span>
